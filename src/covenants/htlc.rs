...
//
// These types are the canonical wire shapes used by `exfer-walletd` and
// `exfer-indexer` to expose HTLC lifecycle data over JSON-RPC. They live in
// the upstream crate so both downstream services depend on identical
// definitions — schema drift between them is impossible.
//
...
pub enum HtlcState {
    ...
    Reclaimed,
    /// Walletd has heard of this lock_tx_id but the follower has not yet
    /// classified the outpoint. Transient.
    Unknown,
}

/// Relationship of the observing wallet/indexer to an HTLC.
#[derive(Serialize, Deserialize, Debug, Clone, Copy, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
pub enum HtlcRole {
    ...
    Both,
    /// Indexer-only: neither key is owned by the observer.
    Observer,
}
...
/// length constraint on the input — a preimage is any byte string
/// whose hash equals the lock. Recording it as raw `Vec<u8>` (rendered
/// as lowercase hex on the JSON wire) is the only shape that can
/// faithfully round-trip every valid claim, including the
/// 29-byte test vector at `src/bin/htlc_test.rs`.
#[derive(Serialize, Deserialize, Debug, Clone, PartialEq, Eq)]
...
/// A single HTLC's observable record, as exposed via walletd or indexer
/// JSON-RPC.
#[derive(Serialize, Deserialize, Debug, Clone, PartialEq, Eq)]
pub struct HtlcRecord {
    ...
    pub amount: u64,
    /// Block height that confirmed the lock. `None` if the lock is still in
    /// the mempool (walletd-side optimistic record).
    pub lock_block_height: Option<u64>,
    pub state: HtlcState,
    pub claim: Option<HtlcClaimRecord>,
    pub reclaim: Option<HtlcReclaimRecord>,
    pub role: HtlcRole,
    /// Tip height seen by the indexer when this record was last written.
...
/// Variable-length `Vec<u8>` <-> lowercase hex. Used for fields whose
/// length is not fixed by the protocol (HTLC preimage).
mod hex_bytes_serde {
    use serde::{Deserialize, Deserializer, Serializer};
...
mod tests {
    ...
    fn htlc_record_round_trips_through_json() {
        ...
        // Spot-check that bytes serialise as lowercase hex strings (the wire
        // convention used by the rest of the JSON-RPC surface).
        ...
        assert_eq!(
            pre_hex,
            hex::encode(b"exfer htlc test preimage 2026"),
        );
    }
    ...
    fn htlc_claim_record_serialises_preimage_as_lowercase_hex() {
        ...
        let v: serde_json::Value = serde_json::from_str(&serde_json::to_string(&record).unwrap()).unwrap();
